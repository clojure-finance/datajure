...
- **`:set :by` window mode is dramatically faster and no longer OOMs on wide datasets.** For a keyword-only `:by` with map derivations, the window-mode group path no longer builds a materialised sub-dataset per group over *all* columns and concatenates them (which dragged passthrough columns through every group and masked-OOM'd on wide data). It now computes the grouped + `:within-order`-sorted permutation once, reorders all columns in a single `ds/select-rows`, and computes each derivation against the reordered base — window ops per contiguous group slice, group aggregators broadcast per group, element-wise derivations once over the whole base. On a real 2.1M-row × 45k-firm-group transform (40 moving averages), a wide `:set` (528 passthrough columns) went from a masked OOM to ~46 s, and the narrowed case from ~137 s to ~42 s. Same results (grouped + within-order order) as before; marker/fn `:by`, vector (sequential) derivations, and plain-fn derivations keep the general path.
