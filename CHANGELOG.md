...
- **Off-heap (native) result columns for `:set :by` — now the default.** For the keyword-only `:by` window-mode `:set` fast path, numeric derived columns are materialised in off-heap native buffers instead of on the JVM heap. Each derived column is packed normally (so its dtype + missing-set match the on-heap result exactly — output is identical *by construction*, and the native datatype is **type-preserving**: integer window ops like `win/rank` or a lag of an int column stay `:int64`, floats stay `:float64`), then its buffer is relocated off-heap, GC-tracked (freed when the dataset is collected, no manual cleanup) and Arrow-exportable. On a real 2.1M-row × 45k-firm transform (40 moving averages) retained JVM heap dropped from ~888 MB to ~13 MB (~68×) at the same speed; extrapolated to a ~270-column per-firm transform that's ~6 GB → ~90 MB, making datajure `:set :by` memory-competitive with a hand-rolled off-heap scatter. Pass **`:off-heap false`** for on-heap output; non-numeric derived columns stay on-heap regardless.
...
- **Non-expanding (R `rollapplyr`) windows via an options map on the rolling ops.** `win/mavg`/`win/msum`/`win/mdev`/`win/mdowndev`/`win/mmin`/`win/mmax` now accept a trailing options map: `#dt/e (win/mavg :ret 12 {:min-periods 12})` requires a full `width`-row window before emitting (leading `width-1` rows → nil), matching R's `zoo::rollapplyr`. Default stays expanding (`:min-periods 1`, the q convention). `win/mdev` also accepts `:ddof` in the map (`{:ddof 0 :min-periods 12}`), with the positional `(win/mdev :col n d)` kept as a back-compat convenience.
...
- **`:set :by` window mode is dramatically faster and no longer OOMs on wide datasets.** For a keyword-only `:by` with map derivations, the window-mode group path no longer builds a materialised sub-dataset per group over *all* columns and concatenates them (which dragged passthrough columns through every group and masked-OOM'd on wide data). It now computes the grouped + `:within-order`-sorted permutation once, reorders all columns in a single `ds/select-rows`, and computes each derivation against the reordered base — pure single-column window ops via zero-copy `dtype/sub-buffer` slicing of the source column per contiguous group (no per-group sub-datasets), group aggregators broadcast per group, element-wise derivations once over the whole base. On a real 2.1M-row × 45k-firm-group transform (40 moving averages), a wide `:set` (528 passthrough columns) went from a masked OOM to ~30 s, and the narrowed case from ~137 s to ~27 s (and wide ≈ narrow — passthrough columns are no longer dragged through the per-group split). Same results (grouped + within-order order) as before; marker/fn `:by`, vector (sequential) derivations, and plain-fn derivations keep the general path.
