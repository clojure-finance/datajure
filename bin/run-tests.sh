#!/usr/bin/env bash
set -e
clojure -Sdeps '{:paths ["src" "resources" "test"] :deps {org.clojure/clojure {:mvn/version "1.12.4"} techascent/tech.ml.dataset {:mvn/version "8.024"}}}' \
  -M -e "
...
(load-file \"test/datajure/index_test.clj\")
(load-file \"test/datajure/math_test.clj\")
(let [result (t/run-tests
               ...
               'datajure.index-test 'datajure.math-test)]
